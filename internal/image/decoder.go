...
// encodeToBase64URI pre-calculates length, allocates a single buffer, and uses zero-copy string conversion
func encodeToBase64URI(mimeType string, data []byte) string {
...
func ReadImage(filePath string) (string, error) {
	...
	if ext == ".raf" {
		...
		return encodeToBase64URI("image/jpeg", rafData.Jpeg), nil
	}

	// ⚡ Bolt Performance Optimization:
	// Fast path for web-supported formats: read directly and encode to base64
	// This skips unnecessary and extremely slow `image.Decode` and `png.Encode` steps
	// for formats the browser can already render natively.
	var mimeType string
	isWebSupported := true
	switch ext {
	case ".jpg", ".jpeg":
	...
	if isWebSupported {
		...
		return encodeToBase64URI(mimeType, data), nil
	}
	...
	if decodeErr != nil {
		...
		return encodeToBase64URI("application/octet-stream", data), nil
	}
	...
	return encodeToBase64URI("image/png", buf.Bytes()), nil
}
